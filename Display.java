...
public class Display
{
  ...
  /**
   * Keeps track of whether the mouse is currently held down. Used to call {@link DisplayableWindow#doDrag(Point2D)}.
   */
  ...
  public Display(String windowName, DisplayableWindow toDisplayFirst)
  {
    ...
    setDisplay(toDisplayFirst);
  }

  public Display(int width, int height, String windowName, DisplayableWindow toDisplayFirst)
  {
  ...
  public Display(int width, int height, String windowName, DisplayableWindow toDisplayFirst, boolean resizeable)
  {
    this.WINDOWWIDTH = width;
    this.WINDOWHEIGHT = height;
    this.ALLOWRESIZE = resizeable;
    this.clearColor = null;
    this.windowName = windowName;
    setDisplay(toDisplayFirst);
  }

  public Display(int width, int height, String windowName, DisplayableWindow toDisplayFirst, Color background)
  {
    this.WINDOWWIDTH = width;
    this.WINDOWHEIGHT = height;
    this.ALLOWRESIZE = true;
  ...
  public Display(int width, int height, String windowName, DisplayableWindow toDisplayFirst, Color background, boolean resizeable)
  {
    this.WINDOWWIDTH = width;
    this.WINDOWHEIGHT = height;
    this.ALLOWRESIZE = resizeable;
   ...
   */
  public void setDisplay(DisplayableWindow display)
  {
    if (currentlyDisplayed != null)
    {
      currentlyDisplayed.parent = null;
      currentlyDisplayed.windowID = -1;
    }
  ...
  private void registerCallbacks()
  {
    ...
    glfwSetMouseButtonCallback(window, (window, button, action, mods) ->
    {
      if (currentlyDisplayed != null)
        ...
        {
          mouseIsDown = true;
          currentlyDisplayed.doClick(button, getCursorLocationOrigin());
        }
        else
          if (action == GLFW_RELEASE)
          {
            mouseIsDown = false;
            currentlyDisplayed.doRelease(button, getCursorLocationOrigin());
          }
    });

    glfwSetScrollCallback(window, (window, xoffset, yoffset) ->
    {
      currentlyDisplayed.doScroll(yoffset, getCursorLocationOrigin());
    });
  }
   ...
   * <p>
   * Loops and calls {@link DisplayableWindow#display()} {@link Display#FPS} times per second, until the window gets closed.
   */
  private void loop()
  {
    ...
    while (!glfwWindowShouldClose(window))
    {
      ...
      if (mouseIsDown)
        currentlyDisplayed.doDrag(getCursorLocationOrigin());
  ...
  {
    double maxSleepUS = 1000000D / updates_per_second;
    double usSinceLastSleep = (System.nanoTime() - last_execution) / 1000D;
    if (maxSleepUS > usSinceLastSleep)
    {
      //System.out.printf("Sleeping for \t"+(long)(maxSleepMS-msSinceLastSleep)+" of \t"+(long)maxSleepMS+"ms.\n");
      try
      {
        Thread.sleep((long) ((maxSleepUS - usSinceLastSleep)/1000));
    ...
    //else
    //  System.err.println("This thread is running behind! This is no time to rest! ("+(long)(usSinceLastSleep-maxSleepUS)+"us behind next scheduled update time.)");
  ...
  private void update()
  {
    ...
    if (clearColor!=null)
      glClearColor((float)clearColor.getRed()/255,(float)clearColor.getGreen()/255,(float)clearColor.getBlue()/255, 0.0f);
    else
    ...
    currentlyDisplayed.parent = this;
    currentlyDisplayed.windowID = window;
  }
   ...
   * Positive y is downward from the origin.
   */
  public static void doPointOr(Point2D pt)
  {
    doPointCart(pt.x - w.getWidth() / 2D, w.getHeight() / 2D - pt.y);
  }
   ...
   */
  public static void drawRectangleOr(double x, double y, int width, int height, boolean fill)
  {
    glBegin(fill ? GL_POLYGON : GL_LINE_LOOP);
    doPointOr(x - width / 2d, y - height / 2d);
    doPointOr(x + width / 2d, y - height / 2d);
    doPointOr(x + width / 2d, y + height / 2d);
    doPointOr(x - width / 2d, y + height / 2d);
    glEnd();
   ...
   */
  public static void drawLineOr(Point2D start, Point2D end)
  {
    glBegin(GL_LINES);
   ...
   * @return The cursor's location in a {@link Point2D}.
   */
  public Point2D getCursorLocationCartesian(WindowSize w)
  {
    try (MemoryStack stack = MemoryStack.stackPush())
    {
      ...
      return new Point2D(x.get() - w.getWidth() / 2D, w.getHeight() / 2D - y.get());
    } catch (IllegalArgumentException e)
    {
      return null;
    }
  ...
  /**
   * Gets the cursor's location, using coordinates from the origin, in double accuracy.
   *
   * @return The cursor's location in a {@link Point2D}.
   */
  public static Point2D getCursorLocationOrigin()
  {
    try (MemoryStack stack = MemoryStack.stackPush())
    {
      //double* x = malloc(1);
